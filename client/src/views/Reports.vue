              ...
              <tr v-for="q in quarterlyData" :key="q.quarter">
                ...
                <td>
                  <span :class="getFulfillmentClass(q.fulfillment_rate)">
                    {{ q.fulfillment_rate != null ? q.fulfillment_rate + '%' : '-' }}
                  </span>
                </td>
              </tr>
            </tbody>
...
export default {
  ...
  setup() {
    ...
    const totalRevenue = computed(() => monthlyData.value.reduce((sum, m) => sum + (m.revenue || 0), 0))
    const avgMonthlyRevenue = computed(() => monthlyData.value.length ? totalRevenue.value / monthlyData.value.length : 0)
    const totalOrders = computed(() => monthlyData.value.reduce((sum, m) => sum + (m.order_count || 0), 0))
    ...
    // Reload whenever any filter changes — same pattern as Orders/Dashboard
    ...
    const maxRevenue = computed(() => Math.max(0, ...monthlyData.value.map(m => m.revenue || 0)))
